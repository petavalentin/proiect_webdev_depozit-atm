		...
		<div class="container">
			...
			<div class="content">
				<?php
					...
					else   //verificare existenta in baza de date
					{
						...
						if($num_rows == 0)   //NU exista inregistrarea in baza de date  =>  afisare mesaj de eroare
						{							
							echo '
								<form action="eliminate2.php" method="post">
									<table style="width: 300px">
							...
							echo '<script type="text/javascript">
									alert("Produsul ' . $_REQUEST['product_name'] . ' nu exista in baza de date!");
								   </script>';
						}
						else  //inregistrarea exista  =>  update baza de date
						{
							...
							//scadere numar bucati produs curent cu numar bucati eliminate
							$nrb -= $_REQUEST['nr_buc'];
							
							//updatarea propriu-zisa a tabelei
							mysql_query("UPDATE produs
										 SET um = '" . $_REQUEST['um'] . "', numar = " . $nrb . "
										 WHERE nume = '" . $_REQUEST['product_name'] . "'");
										 					
								
							//PAS 2:  inserare inregistrare noua in tabela 'log_produs'
							//aflare pret produs de eliminat pentru inserare informatii in tabela 'log_produs'
							$pret_result = mysql_query("select pret from produs
												       where nume='" . $_REQUEST['product_name'] . "'");
							
							$pret_row = mysql_fetch_array($pret_result);
							$pret = $pret_row['pret'];
										
							mysql_query("INSERT INTO log_produs (categorie, nume, um, bucati, pret, data)
										 VALUES (0, '" . $_REQUEST['product_name'] . "', '" . $_REQUEST['um'] . "', " . $_REQUEST['nr_buc'] . ", " . $pret . ", '" . $_REQUEST['data'] . "')");
							...
							echo '<script type="text/javascript">
									alert("Produsul a fost eliminat cu succes din baza de date!");
								   </script>';	
						}		
